...
async def callback_query(call):
        if call.data == "lang_ru":
            ...
            await bot.send_photo(call.from_user.id, photo=open('./banner/banner-main.png', 'rb'), caption='Сардэчна запрашаем у ezcode', reply_markup=await gen_markup('main_by'), timeout=1000000)
            ...
            if get_info['language'] == 'ru':
                ...
                await bot.send_photo(call.from_user.id,photo=open('./banner/banner-main.png', 'rb'), caption='*ezcode officials* \- курсы, распрацаваныя адміністрацыяй ezcode\nМы на 100% упэўнены, што яны навучаць вас новым праграмавальным навыкам\n\n*Выберыце курс*', parse_mode='MarkdownV2', reply_markup=await gen_markup('courses_official', courses=courses_off_list), timeout=1000000000)
                ...
                if get_info['language'] == 'en':
                    await bot.send_photo(call.from_user.id,photo=open('./banner/banner-main.png', 'rb'), caption=f"Name: {await escape_symbols(course_info['content'][0]['name'])}\nDescription: {await escape_symbols(course_info['content'][0]['desc'])}", parse_mode='MarkdownV2',reply_markup = await gen_markup('course_official_steps', course_info),timeout=1000000000)
                elif get_info['language'] == 'ru':
                    await bot.send_photo(call.from_user.id,photo=open('./banner/banner-main.png', 'rb'), caption=f"Название: {await escape_symbols(course_info['content'][0]['name'])}\nОписание: {await escape_symbols(course_info['content'][0]['desc'])}", parse_mode='MarkdownV2',reply_markup = await gen_markup('course_official_steps', course_info), timeout=1000000000)
                elif get_info['language'] == 'by':
                    await bot.send_photo(call.from_user.id,photo=open('./banner/banner-main.png', 'rb'), caption=f"Назва: {await escape_symbols(course_info['content'][0]['name'])}\nАпiсанне: {await escape_symbols(course_info['content'][0]['desc'])}", parse_mode='MarkdownV2', reply_markup = await gen_markup('course_official_steps', course_info), timeout=1000000000)
        elif call.data == 'ver_courses':
                        courses_ver_list = await parse_verified_courses(str(call.from_user.id))
                        get_info = xata.records().get('users', call.from_user.id)
                        if get_info['language'] == 'ru':
                            await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption='*ezcode verified* \- курсы, разработанные доверенными авторами для ezcode\n\n*Выберите курс*', parse_mode='MarkdownV2', reply_markup=await gen_markup('courses_verify', courses=courses_ver_list), timeout=1000000000)
                        elif get_info['language'] == 'en':
                            await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption='*ezcode verified* \- courses developed by verfied authors especially for ezcode\n\n*Choose a course*', parse_mode='MarkdownV2', reply_markup=await gen_markup('courses_verify', courses=courses_ver_list), timeout=1000000000)
                        elif get_info['language'] == 'by':
                            await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption='*ezcode verified* \- курсы, распрацаваныя верыфікаваныя аўтары для ezcode\n\n*Выберыце курс*', parse_mode='MarkdownV2', reply_markup=await gen_markup('courses_verify', courses=courses_ver_list), timeout=1000000000)
        elif 'verify_' in call.data:
                            course_id = call.data
                            course_id = course_id.replace('verify_', '')
                            ...
                            get_info = xata.records().get('users', call.from_user.id)
                            if get_info['language'] == 'en':
                                await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption=f"*Name:* {await escape_symbols(course_info['content'][0]['name'])}\n*Description:* {await escape_symbols(course_info['content'][0]['desc'])}\n\n*Author:* tg://user?id\={str(course_info['user_id'])}",parse_mode='MarkdownV2', reply_markup = await gen_markup('course_verify_steps', course_info),timeout=1000000000)
                            elif get_info['language'] == 'ru':
                                await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption=f"*Название:* {await escape_symbols(course_info['content'][0]['name'])}\n*Описание:* {await escape_symbols(course_info['content'][0]['desc'])}\n*Автор:* tg://user?id\={str(course_info['user_id'])}",parse_mode='MarkdownV2', reply_markup = await gen_markup('course_verify_steps', course_info), timeout=1000000000)
                            elif get_info['language'] == 'by':
                                await bot.send_photo(call.from_user.id,photo=open('./banner/banner-verify.png', 'rb'), caption=f"*Назва:* {await escape_symbols(course_info['content'][0]['name'])}\n*Апiсанне:* {await escape_symbols(course_info['content'][0]['desc'])}\n*Аўтар:* tg://user?id\={str(course_info['user_id'])}",parse_mode='MarkdownV2', reply_markup = await gen_markup('course_verify_steps', course_info), timeout=1000000000)
            ...
            if type == 'off':
                ...
                if len(final_text) >= 4096:
                                        ...
                                        for text in final_text:
                                                await bot.send_message(call.from_user.id, text, parse_mode='MarkdownV2', reply_markup=await gen_markup(f'back_{type}', course_db))
                                                time.sleep(0.1)
                else:
                    try:
                                        await bot.send_message(call.from_user.id, final_text, parse_mode='MarkdownV2',reply_markup=await gen_markup(f'back_{type}', course_db))
                    except:
                        await bot.send_message(call.from_user.id, f"{final_text}*", parse_mode='MarkdownV2',reply_markup=await gen_markup(f'back_{type}', course_db))
            elif type == 'ver':
                    courses_ver_list = await parse_verified_courses(str(call.from_user.id))
                    course_info = []
                    ...
                    if len(final_text) >= 4096:
                        ...
                        try:
                            ...
                            await bot.send_message(call.from_user.id, basic_pro, parse_mode="MarkdownV2", reply_markup=await gen_markup(f'back_{type}', course_db))
